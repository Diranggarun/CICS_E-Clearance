...
export default function ManageFines() {
  ...
  // Auto-load fines whenever the student changes.
  useEffect(() => {
    if (!studentId) {
      setRows([])
      return
    ...
    listFines(studentId)
      ...
      .catch((err) => {
        if (!cancelled) toast.error(err.response?.data?.message || 'Lookup failed.')
      })
  ...
  const add = async (e) => {
    ...
    if (!reason || !amount) return toast.error('Reason and amount are required.')
    setAdding(true)
    try {
...
=======
      ...
      <div className={`${card} p-6 space-y-5`}>
        ...
        {studentId && (
          <form onSubmit={add} className="grid gap-3 md:grid-cols-[2fr_1fr_auto]">
            <input
              placeholder="Reason (e.g. Library book overdue)"
              value={reason}
            ...
            <button
              disabled={adding}
              className="rounded-full bg-[#0D27F7] px-6 py-3 text-sm font-semibold text-white hover:bg-[#0a1fcc] disabled:opacity-60 flex items-center justify-center gap-2"
            >
              <FiPlus /> {adding ? 'Adding…' : 'Add Fine'}
            </button>
          </form>
        )}
      </div>
      ...
      {studentId && (
        <div className={`${card} overflow-hidden`}>
          <div className="flex items-center justify-between border-b border-[#e2ebff] bg-white/60 px-6 py-4">
            <p className="text-sm font-semibold text-[#0D27F7]">
              {rows.length} fine{rows.length === 1 ? '' : 's'} on record
            </p>
            <p className="text-sm font-medium text-gray-600">
              Unpaid total: <span className="font-bold text-[#0D27F7]">₱{totalUnpaid.toFixed(2)}</span>
            </p>
          </div>

          <div className="grid grid-cols-5 border-b border-[#e2ebff] bg-white/40 px-6 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500">
            <p>Reason</p>
            <p>Amount</p>
            <p>Status</p>
            <p>Created</p>
          ...
          {!loading && rows.length === 0 && (
            <p className="px-6 py-10 text-center text-sm text-gray-400">
              No fines on record for this student.
            </p>
          )}
